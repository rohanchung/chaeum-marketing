...
export function printableHTML(r: Report) {
  ...
  const chart = r.trend
    ...
    .join("");
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>로한 마케팅 보고서</title><style>@import url("https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/variable/pretendardvariable-dynamic-subset.min.css");body{font:12px "Pretendard Variable",Pretendard,sans-serif;font-synthesis:none;color:#243b32;margin:28px}button{font:inherit}b,strong{font-weight:700}h1{font-size:26px}h2{font-size:17px;margin-top:28px}table{border-collapse:collapse;width:100%;margin:16px 0}td,th{padding:8px;border-bottom:1px solid #ddd;text-align:left}th{background:#ecf4ef}tr{break-inside:avoid}small{color:#576b60}.cards{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}.card{padding:14px;border:1px solid #ddd;border-radius:8px}.card strong{display:block;font-size:18px;margin-top:8px}.barrow{display:grid;grid-template-columns:95px 1fr 190px;gap:10px;margin:6px 0}.barrow i{display:block;height:5px;min-width:0;margin:2px 0}@page{size:A4 landscape;margin:12mm} @media print{button{display:none}thead{display:table-header-group}}</style></head><body><button onclick="document.fonts.ready.then(() => window.print())">인쇄 / PDF로 저장</button><h1>로한 마케팅 · 기간 보고서</h1><p>${esc(r.period.start)} ~ ${esc(r.period.end)} · 생성 ${esc(r.generatedAt)}</p><div class="cards">${summaryRows(
    r,
  )
    .map(
